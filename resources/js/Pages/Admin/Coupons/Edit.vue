...
const form = reactive({
    ...
    valid_from: props.coupon.valid_from ? props.coupon.valid_from.split('T')[0] : '',
    valid_until: props.coupon.valid_until ? props.coupon.valid_until.split('T')[0] : '',
    is_active: props.coupon.is_active,
});

function submit() {
    router.post(route('admin.coupons.update', { coupon: props.coupon.id }), form, {
