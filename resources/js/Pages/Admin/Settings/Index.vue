    ...
    <div class="px-4 py-6 sm:px-0">
        ...
        <div class="space-y-8">
            <form @submit.prevent="save" class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg p-6 max-w-lg">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Shipping & Tax</h2>
                <div class="space-y-6">
                    <div>
            ...
            <form @submit.prevent="save" class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg p-6 max-w-lg">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Stripe Keys</h2>
                <div class="space-y-6">
                    <div>
                        <label for="stripe_key" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Publishable Key</label>
                        <input id="stripe_key" v-model="form.stripe_key" type="text" placeholder="pk_live_..."
                            class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-sm">
                        <p v-if="errors.stripe_key" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.stripe_key }}</p>
                    </div>

                    <div>
                        <label for="stripe_secret" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Secret Key</label>
                        <input id="stripe_secret" v-model="form.stripe_secret" type="password" placeholder="sk_live_..."
                            class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-sm">
                        <p v-if="errors.stripe_secret" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.stripe_secret }}</p>
                    </div>

                    <div>
                        <label for="stripe_webhook_secret" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Webhook Secret</label>
                        <input id="stripe_webhook_secret" v-model="form.stripe_webhook_secret" type="password" placeholder="whsec_..."
                            class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-sm">
                        <p v-if="errors.stripe_webhook_secret" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.stripe_webhook_secret }}</p>
                    </div>
                </div>
            ...
            <form @submit.prevent="save" class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg p-6 max-w-lg">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">PayPal Keys</h2>
                <div class="space-y-6">
                    <div>
                        <label for="paypal_client_id" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Client ID</label>
                        <input id="paypal_client_id" v-model="form.paypal_client_id" type="text" placeholder="A..."
                            class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-sm">
                        <p v-if="errors.paypal_client_id" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.paypal_client_id }}</p>
                    </div>

                    <div>
                        <label for="paypal_secret" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Secret</label>
                        <input id="paypal_secret" v-model="form.paypal_secret" type="password" placeholder="E..."
                            class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 font-mono text-sm">
                        <p v-if="errors.paypal_secret" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.paypal_secret }}</p>
                    </div>

                    <div>
                        <label for="paypal_mode" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Mode</label>
                        <select id="paypal_mode" v-model="form.paypal_mode"
                            class="mt-1 block w-full border border-gray-300 dark:border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
                            <option value="sandbox">Sandbox</option>
                            <option value="live">Live</option>
                        </select>
                        <p v-if="errors.paypal_mode" class="mt-1 text-sm text-red-600 dark:text-red-400">{{ errors.paypal_mode }}</p>
                    </div>
                </div>
